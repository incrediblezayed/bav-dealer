  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return data.isEmpty
      ...
      child: RefreshIndicator(
        onRefresh: () async {
          await ref.read(orderProvider(OrderFamily.productOrders).notifier).getProductPendingOrders();
          await ref.read(orderProvider(OrderFamily.productOrders).notifier).getProductAcceptedOrders();
          await ref.read(orderProvider(OrderFamily.productOrders).notifier).getProductRejectedOrders();
          await ref.read(orderProvider(OrderFamily.productOrders).notifier).getProductDeliveredOrders();
        },
        child: ListView(
          shrinkWrap: true,
          children: data
